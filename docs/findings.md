...
- **[#23](../../issues/23) — the resolvers leave gaps in files we did not
  write.** Zero unmapped bytes on both bundled samples; **495 unmapped spans,
  45.5 kB** on a Hugging Face Parquet. The coverage invariant this design rests
  on has only ever been validated against fixtures this repository generated.
  Rewriting makes it worse: an unclassifiable region can be neither safely
  dropped from the index nor safely scrubbed.
...
## 6. Things worth knowing regardless of which mode you pick
